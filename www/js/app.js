...
// Stage switching
export function toStage(n){
  $$('.stage').forEach(s=>s.classList.toggle('active', s.id === `stage${n}`));
}
window.toStage = toStage;
document.addEventListener('DOMContentLoaded', () => toStage(1));

// Popup & sound references
...
// Close popup & start next-stage timer if needed
closeBtn.addEventListener('click', ()=>{
  popup.classList.add('hidden');
  const cur = +$('.stage.active').id.replace('stage','');
  if(cur === 3){
    // advance to Stage 4 after Stage 3 popup
    startStageTimer(3, () => toStage(4));
  }
  // NOTE: removed any action for cur===4 so Stage 4 stays up
});

// State for timers in stages 2,3,4
const state = {
  2:{secs:180, timer:null},
  3:{secs:180, timer:null},
  4:{secs:180, timer:null}
};

// Render a random task
function showTask(stage){
  ...
  box.animate([{transform:'scale(.9)'},{transform:'scale(1)'}],
              {duration:300, easing:'ease-out'});
}

// Ensure ACCEPT TASK button is present & wired
function ensureAcceptBtn(stage){
  let btn = $(`#stage${stage} .acceptBtn`);
  if(!btn){
    btn = document.createElement('button');
    btn.className = 'acceptBtn';
    btn.textContent = 'ACCEPT TASK';
    $(`#stage${stage} .slot-machine`).append(btn);
    ...
    btn.addEventListener('click', ()=>{
      if(stage === 1){
        ...
        startStageTimer(2, () => toStage(3));
      }
      else if(stage === 3 || stage === 4){
        // show popup for Stages 3 & 4
...
// Start the countdown for a given stage, call done() when it reaches zero
function startStageTimer(stage, done){
  const disp = $(`#timer${stage}`);
  clearInterval(state[stage].timer);
  state[stage].timer = setInterval(()=>{
    state[stage].secs--;
    disp.textContent = fmt(state[stage].secs);
    if(state[stage].secs <= 0){
      clearInterval(state[stage].timer);
      ding.currentTime = 0; ding.play();
      done();
    }
...
// Wire up all controls once DOM is ready
document.addEventListener('DOMContentLoaded', ()=>{
  $$('.stage').forEach(section=>{
    const stage = +section.id.replace('stage','');

    // SPIN button
    section.querySelector('.spinButton').addEventListener('click', ()=>{
      showTask(stage);
      ensureAcceptBtn(stage);
    });

    // Setup timers for stages 2+ 
    if(stage >= 2){
      const plus  = $(`#stage${stage} .plusBtn`);
      const minus = $(`#stage${stage} .minusBtn`);
      const start = $(`#stage${stage} .startTimerBtn`);
      const reset = $(`#stage${stage} .resetTimerBtn`);
      const disp  = $(`#timer${stage}`);

      state[stage].secs = 180;
      disp.textContent = fmt(180);

      plus.addEventListener('click', ()=>{
        state[stage].secs = Math.min(state[stage].secs + 30, 600);
        disp.textContent = fmt(state[stage].secs);
      });
      minus.addEventListener('click', ()=>{
        state[stage].secs = Math.max(state[stage].secs - 30, 0);
        disp.textContent = fmt(state[stage].secs);
      });

      // START button: only auto-advance for stages 2 & 3
      if(stage === 4){
        ...
        start.addEventListener('click', ()=>{
          startStageTimer(stage, () => toStage(stage+1));
        });
      }
